...
async def main(loop, amount_pages):
    ...
    for page_results in subjects:
        for page_result in page_results:
            post = {"title": page_result[2], "url": page_result[0], "author": page_result[1], "text": page_result[4],
                    "price": page_result[3][0], "currency": page_result[3][1]}
            posts.update({"title": post["title"], "author": post["author"]}, post, upsert=True)

    print("Records: ", posts.count())
